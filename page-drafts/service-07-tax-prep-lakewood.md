# Page 7 — Tax preparation, Lakewood

**Map row:** 7 · Service page · **Hub** · spokes: 8 · 11 · 12 · 13 · 14 · 15
**Primary:** tax preparation lakewood · 30/mo
**Secondaries:** tax preparer lakewood · tax preparation lakewood ca · income tax preparation lakewood ca · tax filing service lakewood ca
**Status:** Draft — needs proof before it goes to Hibu

**Why this page exists despite tiny volume.** It is the home-city page for the service that
brings the most people in, it is where the Google Business Profile points, and six city pages
below it need a hub to link up to. It is also the page that fixes an existing problem: your
homepage currently ranks for tax terms in Holbrook, Panorama City, Greenwood and Antioch, which
means Google cannot tell where you are. A clear Lakewood service page is the correction.

**This is a hub.** Its "Areas we serve" section gains a passage and a link the day each city
page ships.

---

## Metadata

**Slug:** `/services/tax-preparation`

**Title options**
1. Tax Preparation in Lakewood, CA | Top Tax Pros — 45 chars
2. Tax Preparer in Lakewood, California | Top Tax Pros — 50 chars

**Pick: option 1.**

**Meta description**
Individual and business tax preparation in Lakewood, CA. Local office, real people, and a
preparer who explains the return before you sign it. Book an appointment. (158 — trim)

Trimmed: "Individual and business tax preparation in Lakewood, CA. A local office, real people,
and a preparer who explains your return before you sign." (143)

---

## H1

**Tax preparation in Lakewood, California**

## Hero (visible)

A local office, a preparer who sits with you, and a return you understand before you sign it.

**Open weeknights until 8pm and Saturdays, year-round. Walk in, book ahead, or drop off.
Se habla español. Most personal returns $295.**

*Four real differentiators in one line, all confirmed. Most independent preparers nearby close at
5pm and disappear in May. Spanish in the office matters in this service area and almost none of
the competing pages state it.*

⚠️ [Confirm the published hours are current.]

**Primary action:** Book an appointment
**Secondary action:** Call (562) 600-7072

## One proof line (visible)

**Over 20 years serving Lakewood. CTEC A045646, registered since 2003. IRS Annual Filing Season
Program participant, listed in the IRS public preparer directory. 4.5 stars across roughly 80
reviews.**

⚠️ **[One live inaccuracy to fix off-site.** A directory listing claims "over 30 years". Founded
2000 is 26 years and CTEC registration since 2003 is 23. Neither supports 30, so that listing
should be corrected — an overstated tenure on a page about honest pricing is the kind of thing a
competitor screenshots. Confirm the review count too: listings show 77 in one place and 81 in
another.]

## Service area line (visible)

Our office is in Lakewood. We also serve Long Beach, Bellflower, Downey, Cerritos, Norwalk and
Paramount.

---

## Collapsed sections

### What we prepare (accordion)

- Individual federal and California returns
- Returns with self-employment or 1099 income
- Rental property
- Multiple state returns [NEEDS FROM YOU: confirm]
- Business returns [link page 6]

### What to bring (accordion)

Photo ID and Social Security cards for everyone on the return, last year's return, all income
documents (W-2, 1099, K-1), and records for anything you plan to deduct. If you are not sure
whether something counts, bring it.

[Link to page 16, the small business checklist, for business clients.]

### What it costs (accordion)

**Most personal returns are $295. Simple returns start at $250.**

*From 163 invoiced returns, Jan 2025 to Aug 2026: median $295. "From $250" alone under-sold you by
about 15% on your highest-volume service, and a specific number reads as a real price where a
"from" reads as a teaser.*

⚠️ **[Confirm what moves it up.** A starting price without the escalators invites a phone call
that ends in disappointment. Self-employment income, rentals, multiple states and investment
sales are the usual ones.]

**Context worth knowing:** $295 sits right at the national benchmark. The last published National
Society of Accountants survey put a standard-deduction 1040 at roughly $273 in February 2025
dollars, so you are at market, not above it — which is worth saying on the page. But two national
chains with offices within a few miles compete hard on price. This page wins on the office, the
people and the explanation, not on the number.

### Hours and appointments (accordion)

**Monday to Friday, 9am to 8pm. Saturday, 10am to 3pm. Closed Sunday. Open year-round.**
**Appointments or walk-ins. Drop-off available. In-person or virtual.**
**English and Spanish.**

⚠️ [Confirm the hours themselves, and whether they extend during the season. Everything else on
these three lines is confirmed.]

**Move this out of the accordion and into the hero.** Weeknights until 8pm and Saturday hours beat
most independent preparers nearby and match the national chains. It is one of the strongest true
differentiators you have and it was not in any draft until now.

### Areas we serve (accordion — grows with each city page)

**Lakewood.** Our office is here.

*[Add a real passage and link for Long Beach when page 8 ships, then Downey (11), Cerritos (12),
Bellflower (13), Norwalk (14) and Paramount (15). Do not pre-write these.]*

---

## FAQ

**Do I need an appointment?**
No. We take appointments and walk-ins both. An appointment means less waiting during the season.

**Can I drop off my documents?**
Yes. Drop them at the Lakewood office and we will call you with any questions.

**Do you e-file?**
[NEEDS FROM YOU — the last open one on this page. Assume yes, but do not assume it in print.]

**What languages do you speak in the office?**
English and Spanish.

**Are you open year-round?**
Yes. We are here in July as well as April, which is not true of the seasonal storefronts.

**What happens if the IRS sends me a notice after you file?**
If the IRS sends you a notice about a return we prepared and signed, send it to us. We will read
it, check it against what was filed, and tell you plainly whether it is right. If it is, we will
prepare the amendment or help you put the response together.

Brian Cook holds an IRS Annual Filing Season Program Record of Completion, which carries limited
representation rights: we can represent you before IRS revenue agents, customer service and the
Taxpayer Advocate Service on a return we prepared and signed. We cannot represent you before
appeals or collections, and we cannot represent you on a return we did not prepare. Where the
matter needs someone with unlimited rights, we will say so and point you to them.

**Can you help if I owe and cannot pay?**
We prepare and file the return accurately and explain exactly what you owe and why. Arranging a
payment plan or negotiating a balance runs through IRS collections, which is outside our
representation rights. We will tell you that plainly rather than take it on.

---

## Bridge CTA (visible)

**Book a time and bring last year's return.**

---

## Images

1. **Hero** — the actual office exterior or interior. For a local page this outperforms any
   stock image, and it also gives Google a genuine location signal.
2. **Team** — real photos of the people clients meet.
3. Never a stock face on a page about a local office.

---

## Internal links

**Out:** pages 6, 16, and later 8, 11, 12, 13, 14, 15.
**In, required:** homepage (prominently — this is the correction for the homepage's scattered
city rankings), pages 4, 5, 6.

---

## Schema

`LocalBusiness` / `TaxPreparationService` with the real street address, hours and phone. Unlike the
other pages, this one should carry the full address — it is the office page.

**Not `AccountingService`.** Schema is a public claim about what the business is, and California
restricts unlicensed use of accounting titles. See page 5.

---

## ⛔ Before this goes to Hibu

1. **The differentiator line.** Without it this page cannot beat the national chains ranking
   above it.
2. ✅ Appointments, walk-ins and drop-off all available. Hours still to confirm as current.
3. Pricing
4. ✅ English and Spanish
5. ✅ Year-round
6. Real office photos and street address
7. E-filing — confirm

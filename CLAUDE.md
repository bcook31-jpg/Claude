...
- **Credential (confirmed 2026-09-07):** CTEC-registered tax preparers. **No CPA on staff.** Never write "CPA", "public accountant" or any variant as a description of the firm — in copy, metadata, or schema markup. Use `ProfessionalService` or `TaxPreparationService` schema types, never `AccountingService`.
...
- **Writing about CPAs is fine and encouraged.** Blog posts explaining what a CPA costs or how the credentials differ are honest and they are the cheapest rankings available. The line is describing the firm, not discussing the profession.
